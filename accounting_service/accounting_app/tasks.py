...
from accounting_app.streaming import get_event_streaming
...
def create_user(_, event):
    user_model = get_serializer(SerializerNames.USER, event['event_version'])(**event['data'])
...
def handle_created_task(_, event):
    ...
    task_model = get_serializer(SerializerNames.TASK, event_version)(**event['data'])
